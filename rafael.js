...
SCRIPT BY RAFAEL [ RECODE ]
BASE ASLI @EHANZ

BELI PANEL MURAH ? VPS MURAH ? RAFAEL SOLUSINYA
...
//=================================================//
module.exports = Rafael = async (Rafael, m, chatUpdate, store) => {
    
  var ownerNumber = [`${nomerOwner}@s.whatsapp.net` ,`${nomerOwner2}@s.whatsapp.net`,`${Rafael.user.jid}`]
...
const sender = m.key.fromMe ? (Rafael.user.id.split(':')[0]+'@s.whatsapp.net' || Rafael.user.id) : (m.key.participant || m.key.remoteJid)
const senderNumber = sender.split('@')[0]
const pushname = m.pushName || `${senderNumber}`
const isBot = botNumber.includes(senderNumber)
const groupMetadata = isGroup ? await Rafael.groupMetadata(m.chat).catch(e => {}) : ''
...
Rafael.readMessages([m.key])
//Presence Online
if (isCmd){
Rafael.sendPresenceUpdate('recording', from)
} else {
Rafael.sendPresenceUpdate('recording', from)
}
 //User 
...
if(isGroup && isBotGroupAdmins) Rafael.sendMessage(from, { delete: m.key})
if(!isGroup) Rafael.chatModify({ clear: { messages: [{ id: m.id, fromMe: sender == botNumber? true : false, timestamp: m.messageTimestamp }] } }, sender, [])
...
isForwarded: true, 
	...
	thumbnailUrl: "https://telegra.ph/file/036643306aac6ac5c912b.jpg",
...
previewType: "PHOTO",
 thumbnailUrl: photo,
 sourceUrl: 'https://chat.whatsapp.com/L73t0dust3NGj8yIgvDhqb'
   }
  },
 text: teks
  };
return Rafael.sendMessage(m.chat, ngel, { quoted: ftoko});
};
...
// Pp ini mah
try {
var ppimg = await Rafael.profilePictureUrl(sender, 'image').catch(_ => 'https://cdn.pixabay.com/photo/2015/10/05/22/37/blank-profile-picture-973460_960_720.png?q=60')
} catch (err) {
console.log(err)
}
...
let me = m.sender
let anu = "https://telegra.ph/file/036643306aac6ac5c912b.jpg";
...
*Selamat Datang Ini Adalah Kazuto-MD*
...
Terima kasih telah menggunakan layanan kami, semoga anda nyaman dan terus menggunakan layanan kami untuk mempermudah hari hari anda !! 
 `
  await sleep(1000)
Rafael.sendMessage(m.chat,{contextInfo, text:teks},{quoted:ftoko})
                                                                              }
}   
...
Rafael.sendMessage(from, {sticker: oti },{quoted: ftoko})
}
//=============
const jan = fuck
...
Rafael.sendMessage(from, {sticker: kocak },{quoted: ftoko})
}
//==========
const pedo = paan
...
Rafael.sendMessage(from, {sticker: prosi },{quoted: ftoko})
}
//==========
const weh = marah
...
Rafael.sendMessage(from, {sticker: jaga },{quoted: ftoko})
}
//==========
const toxc = bad
...
Rafael.sendMessage(from, {sticker: eeh },{quoted: ftoko})
}
//System Expired
_sewa.expiredCheck(Rafael, sewa)
...
sendSticker2(mosi)
Rafael.sendMessage(m.chat, { delete: m.key })
Rafael.sendMessage(m.chat, { delete: m.key })
...
sendSticker4(toxc)
Rafael.sendMessage(from, { delete: m.key })
...
let linkgc = await Rafael.groupInviteCode(from)
...
sendSticker3(awas)
 Rafael.sendMessage(from, { delete: m.key })
 //conn.groupParticipantsUpdate(from, [sender], '-').catch((e) => { setReply(`BOT HARUS JADI ADMIN`) })
}   
...
setTimeout(() => {
if(isBotGroupAdmins) Rafael.sendMessage(from, { delete: m.key })
//Rafael.groupParticipantsUpdate(from, [sender], '-').catch((e) => { setReply(`BOT HARUS JADI ADMIN`) })
}, 2000)
}
}
...
console.log(color('[KICK]', 'red'), color('Received a virus text!', 'yellow'))
  Rafael.sendText(m.chat, `*TANDAI TELAH DIBACA*\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n *Bang yg ngirim virtex nih:* \nwa.me/${sender.split("@")[0]}`)   
if (!isBotGroupAdmins) {return }
if(isOwner) {return}
await Rafael.groupParticipantsUpdate(from, [sender], 'remove')
Rafael.sendMessage(from, { delete: m.key })
await Rafael.sendMessage(`${nomerOwner}@s.whatsapp.net`,{text: `Hai Owner! wa.me/${sender.split("@")[0]} Terdeteksi Telah Mengirim Virtex ${isGroup?`di Group ${groupName}`:''}`})
 }
 }

//AREA FUNCTION NYA
const mentions = (teks, memberr, id) => {
(id == null || id == undefined || id == false) ?  Rafael.sendMessage(from, { text: teks, mentions: memberr, contextInfo: { "mentionedJid": memberr }}):  Rafael.sendMessage(from, {mentions: memberr,text: teks, contextInfo: { "mentionedJid": memberr }},{quoted: m})
}
 ...
 /* ~~~~~~~~~ RESPON USER AFK ~~~~~~~~~ */
        if (m.isGroup && !m.key.fromMe) {
            ...
            if (afk.checkAfkUser(m.sender, _afk)) {
                ...
                Rafael.sendTextWithMentions(m.chat, `@${m.sender.split('@')[0]} telah kembali dari afk`, ftoko)
            }
        }
...
Rafael.sendMessage(from, { react: { text: emot, key: m.key } })	
}
}
...
item1.TEL;waid=${i}:${i}\n
item1.X-ABLabel:Ponsel\n
item2.EMAIL;type=INTERNET:Rafael@gmail.com\n
item2.X-ABLabel:Email\n
item3.URL:https://Rafael.com
...
await Rafael.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: '𝑲𝒂𝒛𝒖𝒕𝒐', mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: vid.url,sourceUrl: `${sgc}`																												
}}, sticker: nah }, { quoted: ftoko })   				
...
.on("finish", async () => {  
await Rafael.sendMessage(from, { audio:  fs.readFileSync(mp3File), mimetype: 'audio/mp4' },{ quoted: ftoko })
...
Rafael.sendMessage(from, {text: yameteh},{quoted: ftoko})
}

const onlyGlimit = async() => {
let kyahh = `Maaf kak @${sender.split('@')[0]} limit game kamu sudah habis!`
Rafael.sendMessage(from, {text: kyahh},{quoted: ftoko})
}
...
Hallo ${pushname}
bot di setting hanya untuk di gunakan
di dalam group , untuk mendapatkan fitur lengkap
silahkan gunakan di dalam group
 
*LIST FITUR PRIVATE CHAT*
...
`
Rafael.relayMessage(from, { scheduledCallCreationMessage: { callType: "AUDIO", scheduledTimestampMs: 1200, title: teks}},{ messageId: participants  })
await sleep(2000)
}
...
Rafael.sendMessage(from,{text:`Limit kamu tersisa ${db.data.users[sender].limit}`}, {quoted: ftoko})
}
...
if (m.sender.startsWith('212')) return Rafael.updateBlockStatus(m.sender, 'block')
// ====== GAME FEATURE ========//
//GAME Caklontong Function
Rafael.caklontong = Rafael.caklontong ? Rafael.caklontong : {}  
if(isGroup && from in Rafael.caklontong){
if(!isGroup) {return} 
const similarity = require('similarity')
const threshold = 0.72
let id = m.chat
 let json = JSON.parse(JSON.stringify(Rafael.caklontong[id][1]))

 if (budy.toLowerCase() == json.jawaban.toLowerCase().trim()) {
 global.db.data.users[m.sender].exp += Rafael.caklontong[id][2]
// global.db.data.users[m.sender].tiketcoin += 1
 setReply(`*Benar!*\n+${Rafael.caklontong[id][2]} XP\n+1500 Money\n${json.deskripsi}`)
 clearTimeout(Rafael.caklontong[id][3])
 delete Rafael.caklontong[id]
 } else if(similarity(budy.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) setReply(`*Dikit Lagi><*`)
// else setReply(`*Salah!*`)
 
}
  //GAME tebak gambar
Rafael.tebakgambar = Rafael.tebakgambar ? Rafael.tebakgambar : {}  
if(isGroup && from in Rafael.tebakgambar){
if(!isGroup) {return} 
const similarity = require('similarity')
const threshold = 0.72
let id = m.chat
 let json = JSON.parse(JSON.stringify(Rafael.tebakgambar[id][1]))
 if (budy.toLowerCase() == json.jawaban.toLowerCase().trim()) {
user.balance += Rafael.tebakgambar[id][2]
 var teks = `*GAME TEBAK GAMBAR*\n\nJawaban Kamu Benar!\n Hadiah : +${Rafael.tebakgambar[id][2]} Money 💸`
   m.reply (teks)
 clearTimeout(Rafael.tebakgambar[id][3])
 delete Rafael.tebakgambar[id]
 } else if(similarity(budy.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) setReply(`*Dikit Lagi!*`)
}

  
//Game Family 100
Rafael.family = Rafael.family ? Rafael.family : {}
if(isGroup && from in Rafael.family){
if(!isGroup) {return} 
let similarity = require('similarity')
let threshold = 0.72 // semakin tinggi nilai, semakin mirip
let id =  m.chat
let room = Rafael.family[id]
...
Rafael.sendMessage(from, {text: `${caption}`, mentions: [room.terjawab + '@s.whatsapp.net']}, {quoted:ftoko}).then(msg => {
 Rafael.family[id].msg = msg
}).catch(_ => _)
if (isWin || isSurrender) delete Rafael.family[id] 
//if (isWin || isSurrender) clearTimeout(200000)
}
//GAME tebak lirik Function
Rafael.tebaklirik = Rafael.tebaklirik ? Rafael.tebaklirik : {}  
if(isGroup && from in Rafael.tebaklirik){
if(!isGroup) {return} 
const similarity = require('similarity')
const threshold = 0.72
let id = m.chat
let json = JSON.parse(JSON.stringify(Rafael.tebaklirik[id][1]))

 if (budy.toLowerCase() == json.jawaban.toLowerCase().trim()) {
user.balance += Rafael.tebaklirik[id][2]
 global.db.data.users[m.sender].exp += 10
   var teks = `*GAME TEBAK LIRIK*\n\nJawaban Kamu Benar!\n Hadiah : +${Rafael.tebaklirik[id][2]} Money 💸\n EXP: +10`
   m.reply (teks)
 clearTimeout(Rafael.tebaklirik[id][3])
 delete Rafael.tebaklirik[id]
 } else if(similarity(budy.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) setReply(`*Dikit Lagi!*`)
// else setReply(`*Salah!*`)
 }
  //GAME tebak kimia Function
Rafael.tebakkimia = Rafael.tebakkimia ? Rafael.tebakkimia : {}  
if(isGroup && from in Rafael.tebakkimia){
if(!isGroup) {return} 
const similarity = require('similarity')
const threshold = 0.72
let id = m.chat
 let json = JSON.parse(JSON.stringify(Rafael.tebakkimia[id][1]))
 let isSurender = /^((me)?nyerah|surr?ender)$/i.test(budy)

 if (budy.toLowerCase() == json.lambang.toLowerCase().trim()) {
user.balance += Rafael.tebakkimia[id][2]
 global.db.data.users[m.sender].exp += 10
   var teks = `*GAME TEBAK KIMIA*\n\nJawaban Kamu Benar!\n Hadiah : +${Rafael.tebakkimia[id][2]} Money 💸`
   m.reply (teks)
 clearTimeout(Rafael.tebakkimia[id][3])
 delete Rafael.tebakkimia[id]
...
if(isGroup && from in Rafael.math){
if(!isGroup) {return} 
//console.log(Rafael.math)
try{
let id = from
if (!(id in Rafael.math) && /^apa hasil dari/i.test(budy)) return setReply('soal itu sudah berakhir')
let math = JSON.parse(JSON.stringify(Rafael.math[id][1]))
if (budy == math.result) {
user.balance += math.bonus
clearTimeout(Rafael.math[id][3])
delete Rafael.math[id]
...
Rafael.tekateki = Rafael.tekateki ? Rafael.tekateki : {}  
if(isGroup && from in Rafael.tekateki){
if(!isGroup) {return} 
const similarity = require('similarity')
const threshold = 0.72
let id = m.chat
 let json = JSON.parse(JSON.stringify(Rafael.tekateki[id][1]))

 if (budy.toLowerCase() == json.jawaban.toLowerCase().trim()) {
user.balance += Rafael.tekateki[id][2]
 var teks = `*GAME TEKATEKI*\n\nJawaban Kamu Benar!\n Hadiah : +${Rafael.tekateki[id][2]} Money 💸`
 setReply(teks)
 clearTimeout(Rafael.tekateki[id][3])
 delete Rafael.tekateki[id]
 } else if(similarity(budy.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) setReply(`*Dikit Lagi!*`)
}
 //GAME Susunkata Function
Rafael.susunkata = Rafael.susunkata ? Rafael.susunkata : {}  
if(isGroup && from in Rafael.susunkata){
if(!isGroup) {return} 
const similarity = require('similarity')
const threshold = 0.72
let id = m.chat
 let json = JSON.parse(JSON.stringify(Rafael.susunkata[id][1]))

 if (budy.toLowerCase() == json.jawaban.toLowerCase().trim()) {
user.balance += Rafael.susunkata[id][2]
   var teks = `*GAME SUSUN KATA*\n\nJawaban Kamu Benar!\n Hadiah : +${Rafael.susunkata[id][2]} Money 💸`
m.reply(teks)
 clearTimeout(Rafael.susunkata[id][3])
 delete Rafael.susunkata[id]
 } else if(similarity(budy.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) setReply(`*Dikit Lagi!*`)
// else setReply(`*Salah!*`)
 
}
//GAME tebak kata Function
Rafael.tebakkata = Rafael.tebakkata ? Rafael.tebakkata : {}  
if(isGroup && from in Rafael.tebakkata){
const similarity = require('similarity')
const threshold = 0.72
let id = from

 let json = JSON.parse(JSON.stringify(Rafael.tebakkata[id][1]))
 if (budy.toLowerCase() == json.jawaban.toLowerCase().trim()) {
user.balance += Rafael.tebakkata[id][2]
setReply(`*TEBAK KATA*

Jawaban Kamu Benar!
Bonus Saldo : +${Rafael.tebakkata[id][2]}
Exp : +999`)  
 clearTimeout(conn.tebakkata[id][3])
 delete Rafael.tebakkata[id]
 } else if(similarity(budy.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) setReply(`*Dikit Lagi!*`)
 
}
//GAME siapa aku Function
Rafael.siapaaku = Rafael.siapaaku ? Rafael.siapaaku : {}  
if(isGroup && from in Rafael.siapaaku){
if(!isGroup) {return} 
const similarity = require('similarity')
const threshold = 0.72
let id = m.chat
 let json = JSON.parse(JSON.stringify(Rafael.siapaaku[id][1]))

 if (budy.toLowerCase() == json.jawaban.toLowerCase().trim()) {
user.balance += Rafael.siapaaku[id][2]
var teks = `*GAME SIAPAKAH AKU*\n\nJawaban Kamu Benar!\n Hadiah : +${Rafael.siapaaku[id][2]} Money 💸`
   m.reply (teks)
 clearTimeout(Rafael.siapaaku[id][3])
 delete Rafael.siapaaku[id]
 } else if(similarity(budy.toLowerCase(), json.jawaban.toLowerCase().trim()) >= threshold) setReply(`*Dikit Lagi!*`)
// else setReply(`*Salah!*`) 
}

        //Game Suit PvP
	    Rafael.suit = Rafael.suit ? Rafael.suit : {}
	    let roof = Object.values(Rafael.suit).find(roof => roof.id && roof.status && [roof.p, roof.p2].includes(sender))
	    if (roof) {
	    let win = ''
	    let tie = false
	    ...
	    Rafael.sendTextWithMentions(m.chat, `@${roof.p2.split`@`[0]} menolak suit, suit dibatalkan`, dev)
	    delete Rafael.suit[roof.id]
	    return !0
	    }
	    roof.status = 'play'
	    roof.asal = m.chat
	    ...
	    Rafael.sendText(m.chat, `Suit telah dikirimkan ke chat

@${roof.p.split`@`[0]} dan 
@${roof.p2.split`@`[0]}

Silahkan pilih suit di chat masing"
klik https://wa.me/${botNumber.split`@`[0]}`, ftoko, { mentions: [roof.p, roof.p2] })
	    if (!roof.pilih) Rafael.sendText(roof.p, `Silahkan pilih \n\nBatu🗿\nKertas📄\nGunting✂️`, ftoko)
	    if (!roof.pilih2) Rafael.sendText(roof.p2, `Silahkan pilih \n\nBatu🗿\nKertas📄\nGunting✂️`, ftoko)
	    roof.waktu_milih = setTimeout(() => {
	    if (!roof.pilih && !roof.pilih2) Rafael.sendText(m.chat, `Kedua pemain tidak niat main,\nSuit dibatalkan`)
	    else if (!roof.pilih || !roof.pilih2) {
	    win = !roof.pilih ? roof.p2 : roof.p
	    Rafael.sendTextWithMentions(m.chat, `@${(roof.pilih ? roof.p2 : roof.p).split`@`[0]} tidak memilih suit, game berakhir`, ftoko)
	    }
	    delete Rafael.suit[roof.id]
	    return !0
	    }, roof.timeout)
	    ...
	    if (!roof.pilih2) Rafael.sendText(roof.p2, '_Lawan sudah memilih_\nSekarang giliran kamu', 0)
	    }
	    if (jwb2 && reg.test(budy) && !roof.pilih2 && !isGroup) {
	    roof.pilih2 = reg.exec(budy.toLowerCase())[0]
	    roof.text2 = budy
	    setReply(`Kamu telah memilih ${budy} ${!roof.pilih ? `\n\nMenunggu lawan memilih` : ''}`)
	    if (!roof.pilih) Rafael.sendText(roof.p, '_Lawan sudah memilih_\nSekarang giliran kamu', 0)
	    }
	    ...
	    Rafael.sendText(roof.asal, `_*Hasil Suit*_${tie ? '\nSERI' : ''}
...
await Rafael.sendMessage(room.a, {text:"Partner tidak di temukan\nKamu telah keluar dari room anonymous"})
                ...
                if (!m.key.fromMe && secreto.find(i => i.sender === sender)) {
                   ...
                   if (dbx.status === 'ENTER-MESSAGE') {
                     if (['conversation', 'extendedTextMessage'].includes(m.type)) {
                       ...
                       Rafael.sendMessage(dbx.pengirim, { text: teks_balasan })
                       m.reply(`Sukses mengirimkan balasan, ingat pesan ini satu kali kirim saja yaa`)
                     } else {
                       var teks_balasan = `Hai kak, kamu menerima pesan balasan nih\n\nPesan Balasannya :\n${budy}\n\n\n_ingat pesan ini satu kali kirim saja yaa_`
                       var pes = await Rafael.sendMessage(dbx.pengirim, { text: teks_balasan })
                       Rafael.sendMessage(dbx.pengirim, { forward: m }, { quoted: pes })
                       m.reply(`Sukses mengirimkan balasan, ingat pesan ini satu kali kirim saja yaa`)
                     }
...
case 'confes':
                case 'menfess': case 'menfes': {
                  ...
                  var num = num.replace(/[^0-9]/gi, '')+'@s.whatsapp.net'
                   var cek = await Rafael.onWhatsApp(num)
...
`                
                   ...
                   if(isImage || isQuotedImage){
										const media = await Rafael.downloadAndSaveMediaMessage(quoted)
                   const anu = await TelegraPh(media) 
                   Rafael.sendMessage(num, {caption: teks, image: {url: anu}, mentions: [sender]})
									  await sleep(2000)
									 Rafael.sendMessage(num, {image: {url: 'https://telegra.ph/file/b089dfb307251ad5e5559.jpg'}, caption: 'Contoh cara membalas pesan rahasia'})
                   } else if(isVideo || isQuotedVideo){
									 const media = await Rafael.downloadAndSaveMediaMessage(quoted)
                   const anu = await TelegraPh(media) 
                   Rafael.sendMessage(num, {caption: teks, video: {url: anu}, mentions: [sender]})
										await sleep(2000)
									 Rafael.sendMessage(num, {image: {url: 'https://telegra.ph/file/0ca9c64720e7728535491.jpg'}, caption: 'Contoh cara membalas pesan rahasia'})
                   } else {
                   Rafael.sendMessage(num, {text: teks, mentions: [sender]})
										await sleep(2000)
									 Rafael.sendMessage(num, {image: {url: 'https://telegra.ph/file/15b96750f11c403727c6e.jpg'}, caption: 'Contoh cara membalas pesan rahasia'})      
                   }
...
let niu = [
"https://telegra.ph/file/036643306aac6ac5c912b.jpg" ]
const menunya = `
Halo ${pushname}
こんにちは、私は Rafael という名前の人が作成した Kazuto-MD Bot WhatsApp です。私が持っている機能を手伝うことができます。🍂

搾乳しているときにあなたを覚えてもらえるように、データベースに登録することを忘れないでください

Please obey the bot rules and dont *spam* the bot because it can get you *banned* and you wont be able to use the bots features any more.

┌ ◦ ᴀʟʟᴍᴇɴᴜ
│ ◦ 
└ ◦ ʀᴜʟᴇs

私の開発者のすべてのソーシャル メディアを忘れずにフォローして、私に関する最新情報を知ってください。🍁`
Rafael.sendMessage(m.chat, {
    react: {
      text: '🔁',
      key: m.key,
...
break
   case 'allmenu':{
if (!isGroup) return onlyGroup()
let hoshino = [ "https://telegra.ph/file/036643306aac6ac5c912b.jpg","https://telegra.ph/file/036643306aac6ac5c912b.jpg" ]
...
let { key } = await Rafael.sendMessage(m.chat, {text: 'wait!!'}, {quoted: ftoko})//Pengalih isu

for (let i = 0; i < ehe.length; i++) {
await Rafael.sendMessage(m.chat, {text: ehe[i], edit: key });
}
...
Rafael.relayMessage(from, { liveLocationMessage: { 
...
if (/image/.test(mime)){
let media = await Rafael.downloadAndSaveMediaMessage(quoted,makeid(5))
await Rafael.updateProfilePicture(from, { url: media })
.then( res => {
setReply(`Sukses`)
fs.unlinkSync(media)
...
await Rafael.groupUpdateSubject(from, q).then((res)).catch((err) => setReply(jsonformat(err)))
setReply(`Sukses Edit Subjek Grup✅`)
}
break
case 'infogc': {
if(!isGroup) return setReply("Hanya bisa di dalam group")
let _meta = await Rafael.groupMetadata(from)
  console.log(_meta)
let _img = await Rafael.profilePictureUrl(_meta.id, 'image') 
...
case  'getppgc':
	...
	setReply(mess.wait)
	try {
	var ppimg = await Rafael.profilePictureUrl(from, 'image')
	} catch (err) {
	console.log(err)
	var ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
	}
	await Rafael.sendMessage(from, { image: { url: ppimg }}, { quoted: ftoko })
	break
...
if (!isBotAdmins) return setReply(mess.only.badmin)
if (!q) return setReply('Text Nya ?')
await Rafael.groupUpdateDescription(from, q).then((res)).catch((err) => setReply(jsonformat(err)))
...
Rafael.groupSettingUpdate(from, 'announcement')
...
setTimeout(() => {
var nomor = m.participant
Rafael.groupSettingUpdate(from, 'not_announcement')
...
await Rafael.groupParticipantsUpdate(from, [users], 'promote')
setReply(`@${users} Telah Menjadi Admin Group😎`)
}
break
case 'demote': {
if (!isGroup) return onlyGroup()
...
if (!isBotAdmins) return setReply(mess.only.badmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+"@s.whatsapp.net"
await Rafael.groupParticipantsUpdate(from, [botNumber], 'demote')
setReply(`@${users} Menjadi Member Biasa`)
}
break
case  'intro': {
if (!isGroup) return onlyGroup()
...
Rafael.sendMessage(from, { text: q ? q : '', mentions: mem }, {quoted: ftoko})
break
case "tagall": {
if (!isOwner && !isAdmins) return setReply(mess.only.admin)
if (!isGroup) return setReply(mess.only.group)
...
Rafael.sendMessage(from, { text: teks, mentions: participants.map(a => a.id) }, { quoted: ftoko })
}
break
case 'kick': {
if (!isOwner && !isAdmins) return setReply(mess.only.admin)
...
if (!isBotAdmins) return setReply(mess.only.badmin)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '')+'@s.whatsapp.net'
await Rafael.groupParticipantsUpdate(from, [users], 'remove')
}
break
case 'add': {
if (!isOwner && !isAdmins) return setReply(mess.only.admin)
...
await Rafael.groupParticipantsUpdate(from, [users], 'add')
}
break
...
if (!m.isGroup) return setReply(mess.only.group)
if (!isBotAdmins) return setReply(mess.only.badmin)
let response = await Rafael.groupInviteCode(from)
Rafael.sendText(from, `https://chat.whatsapp.com/${response}\n\nLink Group : ${groupMetadata.subject}`, ftoko, { detectLink: true })
}
break
...
Rafael.sendMessage(from, { text: jawab, mentions: menst },{quoted: ftoko}) 
}
break
case 'jodohku':{
if (!isGroup) return onlyGroup()
...
case 'caridoi':
   ...
   if (!isGroup) return onlyGroup()
    ...
    setTimeout(() => {
    Rafael.sendContAngel(from, nomerTemen, namTemen, ftoko)
    }, 9000)
    }
    break
            case 'afk':
...
if (!isGroup) return onlyGroup()
let menst = [sender]
Rafael.sendMessage(from, { text: `@${senderNumber}`, mentions: menst })
}
break
case 'listgc': {
...
for (let i of anu) {
let metadata2 = await Rafael.groupMetadata(i)
...
Rafael.sendTextWithMentions(from, teks, ftoko)
}
break
  case 'listpc': {
if(!isOwner) return setReply(mess.only.owner)
...
Rafael.sendMessage(6281316643491 + "@s.whatsapp.net",{image: { url: `https://telegra.ph/file/df778c74a8f0417235acf.jpg`}, caption: text12, mentionedJid:[sender], quoted: ftoko })
}
break
case 'accerror': {
...
let targetNumber = `${q.split(",")[0]}@s.whatsapp.net`;
  ...
  Rafael.sendMessage(targetNumber, {
    text: `*${zhiratxt}*`,
    mentionesJid: [sender]
  }, {
    quoted: ftoko
...
if (!isGroup) return setReply(mess.only.group)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
await Rafael.updateBlockStatus(users, 'block').then((res) => setReply(jsonformat(res))).catch((err) => setReply(jsonformat(err)))
}
break
case 'unban': {
if (!isOwner && !isAdmins) return setReply(mess.only.admin)
if (!isGroup) return setReply(mess.only.group)
let users = m.mentionedJid[0] ? m.mentionedJid[0] : m.quoted ? m.quoted.sender : text.replace(/[^0-9]/g, '') + '@s.whatsapp.net'
await Rafael.updateBlockStatus(users, 'unblock').then((res) => setReply(jsonformat(res))).catch((err) => setReply(jsonformat(err)))
}
break
//============ KE AMANAN ===========/)
case 'antilinkgc':{
...
Rafael.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `𝑲𝒂𝒛𝒖𝒕𝒐`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/33d58a3a7b931d3902642.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url: a}, caption: `${mess.success}` ,footer: `Hasil pencarian dari ${command}`}, { quoted: ftoko })
...
await Rafael.sendImageAsSticker(m.sender, agg.result.sticker[ki], ftoko, { packname: "©Sticker", author: "Created By  kazuto" })
} }
 break
 case 'sc': case 'script':{
      Rafael.relayMessage(m.chat,  {
...
if (!isGroup) return setReply(mess.only.group)
	...
	let poin = 1000
	let id = m.chat
	if (id in Rafael.tebaklirik) return setReply('Masih ada soal belum terjawab di chat ini')
	...
	Hadiah: ${poin} Money
	`.trim()
	Rafael.tebaklirik[id] = [
	await setReply(caption),
	json, poin,
	setTimeout(() => {
	if (Rafael.tebaklirik[id]) 
user.balance -= 200
...
if (!isGroup) return setReply(mess.only.group)
	...
	let poin = 1000
	let id = m.chat
	if (id in Rafael.siapaaku) return setReply('Masih ada soal belum terjawab di chat ini')
...
if (!isGroup) return onlyGroup()
	...
	let poin = 1000
	let id = m.chat
	if (id in Rafael.susunkata) return setReply('Masih ada soal belum terjawab di chat ini')
...
if (!isGroup) return setReply(mess.only.group)
	...
	let poin = 1000
	let id = m.chat
	if (id in Rafael.tekateki) return setReply('Masih ada soal belum terjawab di chat ini')
...
└────    
	...
	`)
	  let id = from
	  if (id in Rafael.math) return setReply('Masih ada soal belum terjawab di chat ini')
...
if (id in Rafael.tebakkata) return setReply('Masih ada soal belum terjawab di chat ini')
...
if (!isGroup) return onlyGroup()
	...
	let poin = 1000
	let id = m.chat
	if (id in Rafael.tebakkimia) return setReply('Masih ada soal belum terjawab di chat ini')
	...
	Hadiah: ${poin} Money
	`.trim()
	Rafael.tebakkimia[id] = [
	await setReply(caption),
	json, poin,
	setTimeout(() => {
	if (Rafael.tebakkimia[id]) 
user.balance -= 200
m.reply(`*GAME TEBAK KIMIA*\n\nWaktu habis!\n𖦹 Jawabannya adalah; *${json.jawaban}*\n𖦹 Saldo kamu dikurangi 200\n𖦹 Sisa Saldo kamu: *${db.data.users[sender].balance.toLocaleString()}*`)
  ...
  case 'tebakgambar':{
	...
	let poin = 2000
	let id = m.chat
	if (id in Rafael.tebakgambar) return setReply('Masih ada soal belum terjawab di chat ini')
  ...
  console.log(tos)
	...
	Rafael.tebakgambar[id] = [
    await Rafael.sendMessage(from, {caption: caption, image: {url: tos.img}}, {quoted: fkontak}),
	tos, poin,
	setTimeout(() => {
	if (Rafael.tebakgambar[id]) 
user.balance -= 200
...
if (!isGroup) return setReply(mess.only.group)
	...
	let tiketcoin = 1500
	let id = m.chat
	if (id in Rafael.caklontong) return setReply('Masih ada soal belum terjawab di chat ini')
...
if (!isGroup) return setReply(mess.only.group)
  ...
  let timeout = 80000
	let id = m.chat
	if (id in Rafael.family) return setReply('Masih ada kuis yang belum terjawab di chat ini') 
	...
	*Soal:* ${json.soal}
	...
	if (!isGroup) return setReply (mess.group)
            ...
            let poin_lose = 10
            let timeout = 60000
            if (Object.values(Rafael.suit).find(roof => roof.id.startsWith('suit') && [roof.p, roof.p2].includes(sender))) setReply(`Selesaikan suit mu yang sebelumnya`)
	...
	if (m.mentionedJid[0] === botNumber) return setReply (`Tidak bisa bermain dengan Bot !`)
            ...
            if (Object.values(Rafael.suit).find(roof => roof.id.startsWith('suit') && [roof.p, roof.p2].includes(m.mentionedJid[0]))) return setReply ( `Orang yang kamu tantang sedang bermain suit bersama orang lain :(`)
...
Silahkan @${m.mentionedJid[0].split`@`[0]} untuk ketik terima/tolak`
            Rafael.suit[id] = {
            chat: await Rafael.sendText(m.chat, caption, ftoko, { mentions: parseMention(caption) }),
            id: id,
            p: sender,
            p2: m.mentionedJid[0],
            status: 'wait',
            waktu: setTimeout(() => {
            if (Rafael.suit[id]) Rafael.sendText(m.chat, `_Waktu suit habis_`, ftoko)
            delete Rafael.suit[id]
...
case 'buyglimit':{
 ...
 case 'me': case 'profil': {
if (!isGroup) return onlyGroup()
	let ppimg = await Rafael.profilePictureUrl(sender, 'image').catch(_ => 'https://telegra.ph/file/24fa902ead26340f3df2c.png')
	try{
	 let sol = await Rafael.fetchStatus(sender)
	...
	Rafael.sendMessage(from, { caption: teks, image: gambar}, {quoted: ftoko})
...
for (let i = 0; i < total; i ++){
   ...
   //Rafael.sendMessage(from, {text: top, mentions: arrTop}, {quoted: ftoko})
...
Atau ketik .buyprem untuk membeli unlimited limit`
 ...
 case 'listonline':
            case 'liston': {
                ...
                Rafael.sendText(m.chat, '⏰ List Online:\n\n' + online.map(v => '🌱 @' + v.replace(/@.+/, '')).join`\n`, m, {
                    mentions: online
                })
            }
            break
...
break
 ...
 case 'ai':
    case 'openai' : {
		if (!q) return setReply(`Ada Yang Bisa 𝑲𝒂𝒛𝒖𝒕𝒐 Bantu?`)			
m.reply(mess.wait)
			var { data } = await axios.get(`https://api.betabotz.org/api/search/openai-logic?text=${q}&logic=Halo%20Nama%20Saya%20Adalah%20𝑲𝒂𝒛𝒖𝒕𝒐%20saya%20akan%20membalas%20c hat%20mu%20dengan%20emote%20sesuai%obrolan%20kita&apikey=2fbgCgOB`)
...
case 'tiktok':
 ...
 await Rafael.sendMessage(m.chat, {video: { url: e.result.link }, caption: ee }, { quoted: ftoko })
 break
        case 'ttmp3': 
case 'tiktokaudio':
            case 'ttaudio': {
...
setReply(mess.wait)
                ...
                Rafael.sendMessage(m.chat, { audio: { url: i.result }, mimetype: 'audio/mp4' }, { quoted: ftoko })
            }
            break
...
Rafael.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true, 
title: `${res.title}`,
body: `${week} , ${calender}`,																													
mediaType: 2,  
renderLargerThumbnail : true,
...
mediaUrl: res.url,
sourceUrl: res.url
}}, image: {url: inithumb},text: teks},{quoted: ftoko})                                        
downloadMp3(`${res.url}`) 
} catch (err){
console.log(err)
...
setReply (mess.wait)
let res = await fetchJson (`https://skizo.tech/api/pinterest?search=${q}&apikey=${api.Rafael}`)
Rafael.sendMessage(from, {image: {url: res[0].media.url}}, {quoted:ftoko})
}
break
        case 'ttpoto':{
      ...
      m.reply('media Akan 𝑲𝒂𝒛𝒖𝒕𝒐 Kirim Lewat Pc')
      for (let i of asd[0].imgSrc) {
           let lol = m.mentionedJid[0] || m.sender || Rafael.parseMention(args[0]) || (args[0].replace(/[@.+-]/g, '').replace(' ', '') + '@s.whatsapp.net') || ''
           Rafael.sendFile(lol, i, '', null, ftoko)
           }
      }
...
Rafael.sendMessage(m.chat, { image: { url: data.result.thumbnail }, caption }).then(() => {
Rafael.sendMessage(m.chat, { video: { url: data.result.link }, mimetype: 'video/mp4', fileName: `${data.result.title}.mp4` })})
})
.catch(console.error)
break
case 'remini':
...
sendSticker(stikot)
                ...
                Rafael.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/upscale?apikey=${api.Lol}&img=${anu}` }, caption: "nih kak >\<" }, { quoted: ftoko })}
            break
...
setReply(mess.wait)
let res = await fetchJson(`https://skizo.tech/api/ttsearch?search=${q}&apikey=${api.Rafael}`)
...
`
Rafael.sendMedia(from, res.play, ftoko, {caption: caption})
...
Rafael.sendMessage(from,{image, caption},{quoted: ftoko})
} 
break
case 'nsfw': {
...
previewType:"PHOTO", 
thumbnail:`https://telegra.ph/file/036643306aac6ac5c912b.jpg`,
sourceUrl:`https://chat.whatsapp.com/L73t0dust3NGj8yIgvDhqb`
}}, image: fs.readFileSync('./media/thumb.jpeg'), caption: anu}, { quoted: ftoko })
}
break
...
Rafael.sendMessage(m.sender, { caption: `Random Anime ${command}`, image: { url: `https://botapi.takimtod.repl.co/api/nsfw/${command}?apikey=BetaBotz`}, quoted: ftoko })
  db.data.users[sender].limit -= 1  
}
break
...
.then(({data}) => {
Rafael.sendImageAsSticker(m.chat, data.url, ftoko, { packname: global.packName, author: global.authorName })
})
break
case 'anime':{
if (!isGroup) return onlyGroup()
...
//let buttons = [{buttonId: `${prefix + command}`,buttonText:{displayText: `➡️ ɴᴇxᴛ ᴘʜᴏᴛᴏ`},type:1}]                       
Rafael.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${TextT}`, mediaType: 3,  renderLargerThumbnail : true,thumbnail:'',sourceUrl: `https://wa.me/${nomerOwner}`																
}}, image: media, caption: `_${command}_` ,footer: `Hasil pencarian dari ${command}`}, { quoted: ftoko })								
/*}}, image: media, caption: `${mess.success}`, buttons: buttons , footer: `Hasil pencarian dari ${command}`}, { quoted: ftoko })*/
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1 
});
})
...
await Rafael.sendFile(m.chat, cita, 'image.jpg', `_${command}_`, ftoko)
...
try{
setReply(mess.wait)
Rafael.sendMessage(from, { image: { url: `https://api.lolhuman.xyz/api/random2/${command}?apikey=${api.Lol}` } })
}catch(err){
...
Rafael.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${TextT}`, mediaType: 3,  renderLargerThumbnail : true,thumbnail:'',sourceUrl: `https://wa.me/${nomerOwner}`																										
}}, image: kentir, caption: `_${command}_` ,footer: `Hasil pencarian dari ${command}`}, { quoted: ftoko })
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1 
}
break
...
Rafael.sendMessage(m.chat, { image: { url: hehe }, caption: `_${command}_`}, { quoted: ftoko})
}
break
 //========== TOOLS ===========//
 case 'tomp4': 
...
if (!isGroup) return onlyGroup()
                ...
                setReply(mess.wait)
                let media = await Rafael.downloadAndSaveMediaMessage(qmsg)
                let webpToMp4 = await webp2mp4File(media)
                await Rafael.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' } }, { quoted: ftoko })
...
if (!isGroup) return onlyGroup()
                ...
                exec(`ffmpeg -i ${media} ${ran}`, (err) => {
                    ...
                    Rafael.sendMessage(m.chat, { image: buffer }, { quoted: ftoko })
                    fs.unlinkSync(ran)
                })
...
if (!isGroup) return onlyGroup()
            ...
            sendSticker(stikot)
            let media = await Rafael.downloadMediaMessage(qmsg)
            let audio = await toAudio(media, 'mp4')
            Rafael.sendMessage(m.chat, {audio: audio, mimetype: 'audio/mpeg'}, { quoted : ftoko })
...
if (!isGroup) return onlyGroup()
	if (/audio/.test(mime)){ 
	setReply(mess.wait)
	let media3 = await Rafael.downloadAndSaveMediaMessage(quoted,makeid(5))
	...
	await Rafael.sendMessage(from, {audio: jadie, mimetype: 'audio/mp4' }, {quoted: ftoko})
	fs.unlinkSync(rname)
...
if (!isGroup) return onlyGroup()
            ...
            Rafael.sendMessage(m.chat, {audio: audio, mimetype:'audio/mpeg', ptt:true }, {quoted:ftoko})
...
if (!isGroup) return onlyGroup()
                ...
                let media = await Rafael.downloadAndSaveMediaMessage(qmsg)
                let webpToMp4 = await webp2mp4File(media)
                await Rafael.sendMessage(m.chat, { video: { url: webpToMp4.result, caption: 'Convert Webp To Video' }, gifPlayback: true }, { quoted: ftoko })
                await fs.unlinkSync(media)
...
if (!isGroup) return onlyGroup()
                ...
                Rafael.sendMessage(m.chat, { audio: buff, mimetype: 'audio/mpeg' }, { quoted : ftoko })
                fs.unlinkSync(ran)
                })
	...
	(m.message.extendedTextMessage) 
{
  var dataVideo = { ptvMessage: m.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage }
    Rafael.relayMessage(m.chat, dataVideo, {})
 }
    } catch (error) {
        m.reply(error);
        }
...
case "fetch":
  if (!q) {
    return m.reply(`Contoh:\n${prefix + command} https://github.com/𝑲𝒂𝒛𝒖𝒕𝒐`);
  }
  if (!/^https?:\/\//.test(q)) {
  ...
  if (body.match(/(mp4)/gi)) {
    fetch(`${q}`, requestOptions)
      .then((res) => Rafael.sendMessage(from, { video: { url: `${q}` }, mimetype: "video/mp4", caption: "Success" }, { quoted: ftoko }))
      .catch((error) => m.reply("Error", error));
  } else if (body.match(/(mp3)/gi)) {
    fetch(`${q}`, requestOptions)
      .then((res) => Rafael.sendMessage(from, { audio: { url: `${q}` }, mimetype: "audio/mp4", fileName: "Audio" }, { quoted: ftoko }))
      .catch((error) => m.reply("Error", error));
  } else if (body.match(/(png|jpg|jpeg)/gi)) {
    fetch(`${q}`, requestOptions)
      .then((res) => Rafael.sendMessage(from, { image: { url: `${q}` }, caption: "Success" }, { quoted: ftoko }))
      .catch((error) => m.reply("Error", error));
  } else {
    fetch(`${q}`, requestOptions)
 ...
 if (!isMedia) return setReply("Where The A Image")
                ...
                let anu = await TelegraPh(media)
                Rafael.sendMessage(m.chat, {
                    image: { url: `https://api.lolhuman.xyz/api/removebg?apikey=${api.Lol}&img=${anu}` },caption: "nih kak>\<" }, { quoted: ftokk}) }
            break
  case 'flaming1':{
if (!isGroup) return onlyGroup()
...
Rafael.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `𝑲𝒂𝒛𝒖𝒕𝒐`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl:"https://telegra.ph/file/33d58a3a7b931d3902642.jpg",sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url:buffer}, caption: `${mess.success}`}, { quoted: ftoko})
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
break
...
Rafael.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnailUrl: 'https://telegra.ph/file/33d58a3a7b931d3902642.jpg',sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url:buffer}, caption: `${mess.success}`}, { quoted: ftoko})
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
...
Rafael.sendMessage(from, { contextInfo: {externalAdReply: {showAdAttribution: true, title: `${botName}`, mediaType: 3,  renderLargerThumbnail : true,thumbnail: fs.readFileSync('./media/thumb.jpeg'),sourceUrl: `https://wa.me/${nomerOwner}`
}}, image: {url:buffer}, caption: `${mess.success}`}, { quoted: ftoko})
.catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
...
case 'emojimix': {
                ...
                for (let res of anu.results) {
                    let encmedia = await Rafael.sendImageAsSticker(m.chat, res.url, m, {
...
let tod = body.split("\n");
let pjr = tod[Math.floor(Math.random() * tod.length)];
Rafael.sendImageAsSticker(from, pjr, ftoko)
}).catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
break
...
let tod = body.split("\n");
let pjr = tod[Math.floor(Math.random() * tod.length)];
Rafael.sendImageAsSticker(from, pjr, m)
}).catch((err) => setReply('Server sedang error coba lagi besok!'))
db.data.users[sender].limit -= 1
}
break
...
case 'smeme': {
	        ...
	        let dwnld = await Rafael.downloadAndSaveMediaMessage(qmsg)
	        let fatGans = await TelegraPh(dwnld)
	        let smeme = `https://api.memegen.link/images/custom/${encodeURIComponent(bawah)}/${encodeURIComponent(atas)}.png?background=${fatGans}`
	        let FaTiH = await Rafael.sendImageAsSticker(m.chat, smeme, ftoko, { packname: global.packName, author: global.authorName })
...
case 'take': case 'swm': {
            ...
            setReply(mess.wait)
                if (/image/.test(mime)) {
                    let media = await Rafael.downloadMediaMessage(qmsg)
                    let encmedia = await Rafael.sendImageAsSticker(m.chat, media, ftoko, { pacname: teks1, author: teks2 })
                    await fs.unlinkSync(encmedia)
                } else if (/video/.test(mime)) {
                    if ((quoted.msg || quoted).seconds > 11) return setReply('Maksimal 10 detik!')
                    let media = await Rafael.downloadMediaMessage(qmsg)
                    let encmedia = await Rafael.sendVideoAsSticker(m.chat, media, ftoko, { packname: teks1, author: teks2 })
                    await fs.unlinkSync(encmedia)
                } else {
 ...
 case 'attp':
                ...
                setReply(mess.wait)
                Rafael.sendMessage(m.chat, { sticker: { url: `https://api.lolhuman.xyz/api/attp?apikey=${api.Lol}&text=${q}`} }, { quoted: ftoko })
                break
case 'ttp':{
	...
	let nono = await jancok.toFile(stok)
	let nah = fs.readFileSync(nono)
	await Rafael.sendMessage(from,{sticker: nah},{quoted: ftoko})
	await fs.unlinkSync(stok)
...
case 'tts':
	...
	if (ras) Rafael.sendMedia(from, ras, ftoko, {caption: "Nih"})
...
Rafael.sendMessage(from,{caption: 'Dosa Tanggung  Sendiri Dah',video:{url: nano.video_1}},{quoted:ftoko})
...
Rafael.sendMessage(m.chat,{video:{url:hayu},caption:`nih kak🗿`},{quoted: ftoko})
}
break 
...
Rafael.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `${week} , ${calender}⌟`,previewType:"PHOTO",thumbnail: its,sourceUrl: `https://www.instagram.com/𝑲𝒂𝒛𝒖𝒕𝒐`,}
}, text: '*Pertanyaan : '+bisakah+'*\n\n*Jawaban :* '+ keh }, { quoted: ftoko })
}
break
case 'gimana':
case 'gimanadong':
...
Rafael.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `${week} , ${calender}ㅤㅤㅤㅤㅤㅤㅤㅤㅤ`,body:`⌜ ${ucapanWaktu} ⌟`,previewType:"PHOTO",thumbnail: its,sourceUrl: `https://www.instagram.com/𝑲𝒂𝒛𝒖𝒕𝒐`,}
}, text: '*Pertanyaan : '+bagaimanakah+'*\n\n*Jawaban :* '+ mana}, { quoted: ftoko })
}
break
case 'apakah':{
if (!isGroup) return m.reply('Kusus group')
...
Rafael.sendMessage(from, { text: 'Pertanyaan : *'+watak+'*\n\nJawaban : '+ tak}, { quoted: ftoko })
break				
    case 'hobbycek': case 'cekhobby':
const hobby = body.slice(0)
const hob =['Memasak','Membantu Atok','Mabar','Nobar','Sosmedtan','Membantu Orang lain','Nonton Anime','Nonton Drakor','Naik Motor','Nyanyi','Menari','Bertumbuk','Menggambar','Foto fotoan Ga jelas','Maen Game','Berbicara Sendiri']
const by = hob[Math.floor(Math.random() * hob.length)]
Rafael.sendMessage(from, { text: 'Pertanyaan : *'+hobby+'*\n\nJawaban : '+ by}, { quoted: ftoko })
break
...
Rafael.sendMessage(from, { contextInfo: { externalAdReply: { showAdAttribution: true,
title: `𝑲𝒂𝒛𝒖𝒕𝒐`,body:`⌜ ${ucapanWaktu} ⌟`,previewType:"PHOTO",thumbnail: its,sourceUrl: `https://www.instagram.com/𝑲𝒂𝒛𝒖𝒕𝒐`,}
}, text: '*Pertanyaan : '+edgar+'*\n\n*Jawaban :* '+ pret}, { quoted: ftoko })
}
break
    case 'cekmemek':
			case 'cekmeki':
...
text : 'cekkomtlo🗿: *'+persenbucin+'*\n\nJawaban : '+ ehan}, { quoted: ftoko })
				break 

  case 'cekme':{
if (!isGroup) return setReply(mess.only.group)
let ppimg = await Rafael.profilePictureUrl(sender, 'image').catch(_ => 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg')
...
}, image: its, caption: teks}, { quoted: ftoko })
}
...
break
case 'fengshui': {
if (!q) return setReply( `Example : ${prefix + command} 𝑲𝒂𝒛𝒖𝒕𝒐, 1, 2005\n\nNote : ${prefix + command} Nama, gender, tahun lahir\nGender : 1 untuk laki-laki & 2 untuk perempuan`)
...
│• kurang
╰─────────────────✧`
Rafael.sendMessage(from, {text: text12}, {quoted: m})
}
break
case 'tambah':{
...
break
case 'runtime':{
setReply(`*𝑲𝒂𝒛𝒖𝒕𝒐 Aktif* ${runtime(process.uptime())}`)
}
break
case 'owner': {
const repf = await Rafael.sendMessage(from, { 
contacts: { 
...
Rafael.sendMessage(from, { text : `Hai Kak @${sender.split("@")[0]}, Hii itu nomor owner ku jan spam yak:)`, contextInfo:{
forwardingScore: 9999999, 
isForwarded: true,
mentionedJid:[sender]
}}, { quoted: ftoko })
...
await Rafael.updateProfileStatus(q)
...
/*case 'setppbot':
                ...
                var medis = await Rafael.downloadAndSaveMediaMessage(quoted, 'ppbot.jpeg')
                if (args[0] == 'full') {
                    var {
                        img
                    ...
                    var memeg = await Rafael.updateProfilePicture(botNumber, {
                        url: medis
                    })
                    fs.unlinkSync(medis)
                    setReply(mess.succes)
...
case 'bucinsertifikat':
			...
			case 'bucinserti':
				...
				setReply(mess.wait)
				let kueri = args.join(" ")
                Rafael.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/bucinserti?apikey=${api.Lol}&name=${kueri}`}, caption: 'Sertifikatnya kack'}, {quoted: ftoko})
            break
case 'tololserti':
			case 'tololcert':
			...
			setReply(mess.wait)
			let ytta = args.join(" ")
            Rafael.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/toloserti?apikey=${api.Lol}&name=${ytta}`}, caption: 'Sertifikatnya kack'}, {quoted: ftoko})
            break
case 'pacarsertifikat':
            ...
            Rafael.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/pacarserti?apikey=${api.Lol}&name1=${nik}&name2=${prov}`}, caption: titidnya}, {quoted: ftoko})
            break
case 'pornhub':
		case 'glitch':
		case 'avenger':
		...
		case 'wallgravity':
			...
			setReply(mess.wait)
			Rafael.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome2/${command}?apikey=${api.Lol}&text1=${args[0]}&text2=${args[1]}`}, caption: `© kazutoz\n\n Type: ${command}`})
			break
case 'blackpink':
		case 'neon':
		case 'greenneon':
		...
		case 'thunder':
			...
			setReply(mess.wait)
			Rafael.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/textprome/${command}?apikey=${api.Lol}&text=${args}` }, caption: `© Hoshino Botz\n\n Type: ${command}\n\nText: ${args}`})
			break
case 'cyberspace':
            case 'mountain':
            case 'programming':
            ...
            setReply(mess.wait)
                ...
                Rafael.sendMessage(m.chat, { image: { url: result}, caption: mess.succes }, { quoted: ftoko })
            }
            break
case 'cecan': case 'cogan': case 'hacker': case 'pubg': case 'boneka':
if (!isGroup) return onlyGroup()
setReply(mess.wait)
Rafael.sendMessage(from, { image: { url: `https://api.zahwazein.xyz/randomimage/${command}?apikey=${api.angel}` } })
break
...
case 'jam':
  ...
  Rafael.sendText(sender, `Sekarang pukul ${formattedTime} WIB.`);
  break;
case 'wattpad': {
if (!isGroup) return onlyGroup()
...
if (!isGroup) return onlyGroup()
                ...
                Rafael.sendMessage(m.chat, { image: { url: random.male }, caption: `Couple Male` }, { quoted: m })
                Rafael.sendMessage(m.chat, { image: { url: random.female }, caption: `Couple Female` }, { quoted: ftoko })
            }
	    break	    
        case 'pantun': {
if (!isGroup) return onlyGroup()
...
if (!isGroup) return onlyGroup()
let 𝑲𝒂𝒛𝒖𝒕𝒐 = await fetch(`https://api.betabotz.org/api/random/${command}?apikey=2fbgCgOB`)
let hasil = await 𝑲𝒂𝒛𝒖𝒕𝒐.json()
 setReply(`${hasil.result}`.trim())
 }
 break
case 'infocuaca':{
			...
			Rafael.sendMessage(m.chat, { location: { degreesLatitude: data.result.latitude, degreesLongitude: data.result.longitude } })
  ...
  const participants = await Rafael.groupMetadata(m.chat).then(metadata => metadata.participants);
 ...
 case 'memeindo':
                setReply(mess.wait)
                Rafael.sendMessage(m.chat, { image: { url: `https://api.lolhuman.xyz/api/meme/memeindo?apikey=${api.Lol}`}, caption: `sangad Cringe Kaya idup Lo🗿`}, {quoted: m})
            break
case 'tiktokghea':
setReply(mess.wait)
var gheayubi = JSON.parse(fs.readFileSync('./HostMedia/tiktokvids/gheayubi.json'))
var hasil = pickRandom(gheayubi)
Rafael.sendMessage(m.chat, { caption: mess.success, video: { url: hasil.url }}, { quoted: ftoko })
break
case 'cosplayangel':
setReply(mess.wait)
var gheayubi = JSON.parse(fs.readFileSync('./HostMedia/tiktokvids/cosplayangel.json'))
var hasil = pickRandom(gheayubi)
Rafael.sendMessage(m.chat, { caption: 'nih bang ehanz\ningat jangan Gamon yak\nAng3l udah pergi;)', video: { url: hasil.url }}, { quoted: ftoko })
break
case 'tiktokbocil':
setReply(mess.wait)
...
var hasil = pickRandom(notnot)
Rafael.sendMessage(m.chat, { caption: mess.success, image: { url: hasil.url } }, { quoted: ftoko })
break
case 'hijab':
setReply(mess.wait)
...
let encmedia = await Rafael.sendImageAsSticker(from, media, ftoko, { packname: global.packName, author: global.authorName })
await fs.unlinkSync(encmedia)
} else if (/video/.test(mime)) {
if ((quoted.msg || quoted).seconds > 11) return setReply('Maksimal 10 detik!')
let media = await quoted.download()
let encmedia = await Rafael.sendVideoAsSticker(from, media, ftoko, { packname: global.packName, author: global.author })
await fs.unlinkSync(encmedia)
} else {
...
const text = `${teks}`
const username = await Rafael.getName(m.quoted ? m.quoted.sender : m.sender)
const avatar = await Rafael.profilePictureUrl( m.quoted ? m.quoted.sender : m.sender,"image").catch(() =>`https://i0.wp.com/telegra.ph/file/134ccbbd0dfc434a910ab.png`)
...
let encmedia = await Rafael.sendImageAsSticker(m.chat, buffer, ftoko, { packname: global.packName, 
...
Rafael.sendMessage(owner + "@s.whatsapp.net",{image: { url: `https://telegra.ph/file/df778c74a8f0417235acf.jpg`}, caption: text12, mentionedJid:[sender], quoted: ftoko })
}
break
case 'accpesanan': {
if (!isOwner && !isAdmins) return setReply()
...
//======== MENU OWNER ======//
 case 'ban':{
			...
			let Name = await Rafael.getName(woke)
			...
			let Name = await Rafael.getName(users)
...
//	await sleep(2000)
 ...
 case 'restart':
	...
	await Rafael.sendMessage(from, {text: "_Succes_"})
	await sleep(3000)
	process.send('reset') 
	break
 case 'clearsesi':{
...
case 'addprem':
		...
		Rafael.sendText(mentionByTag, `Halo kak aku *${fake}*\nkamu telah terdaftar sabagai user premium\nterimakasih sudah menggunakan ${fake}\n\nSilahkan ketik .cekprem untuk melihat expired premium kamu`)
		   } else if(!isGroup){
		...
		await setReply(`Succses, silakan ketik ${prefix}listprem untuk melihat list premium`)
		Rafael.sendText(usernya, `Halo kak aku *${fake}*\nkamu telah terdaftar sabagai user premium\nterimakasih sudah menggunakan ${fake}\n\nSilahkan ketik .cekprem untuk melihat expired premium kamu`)
...
setReply(premiumnya)
 ...
 case 'addsewa':{
	...
	await Rafael.groupAcceptInvite(kode)
	let { id, subject,creator,creation,desc,descId } = await Rafael.groupGetInviteInfo(kode).catch(async () => {
...
var alala = await Rafael.downloadAndSaveMediaMessage(quoted,makeid(5))
}
...
Rafael.sendMessage(i,{contextInfo,image:fs.readFileSync(alala),caption:text})
} else if(isQuotedVideo || isVideo){
Rafael.sendMessage(i,{contextInfo,video:fs.readFileSync(alala),caption:text})
} else if(isQuotedAudio){
Rafael.sendMessage(i,{contextInfo,Audio:fs.readFileSync(alala)})
} else Rafael.sendMessage(i,{contextInfo,text})
}
...
//conn.sendAdReply(from,tekz,copyright,baileysVersion,link,{quoted:ftoko})
setReply(tekz)
}
break
case 'join':{
	...
	await Rafael.groupAcceptInvite(ano)
...
case 'addvn': {
                ...
                let delb = await Rafael.downloadAndSaveMediaMessage(quoted)
                VnEhanz.push(q)
...
await Rafael.sendMessage(from, { document: fs.readFileSync("./all/database/contacts.vcf"), fileName: "contacts.vcf", caption: "Nih Kak Tinggal Pencet File Di Atas Terus Save", mimetype: "text/vcard", }, { quoted: m })
...
Rafael.sendMessage(m.chat, {contacts: snContact}, {ephemeralExpiration: 86400})
}
break
case "savecontactid": {
if (!isOwner && !isAdmins) return m.reply(mess.only.admin)
...
const groupMetadataa = !isGroup? await Rafael.groupMetadata(`${text}`).catch(e => {}) : ""
const participants = !isGroup? await groupMetadataa.participants : ""
const halls = await participants.filter(v => v.id.endsWith('.net')).map(v => v.id)
for (let mem of halls) {
if (isContacts) return
...
await Rafael.sendMessage(from, { document: fs.readFileSync("./database/contacts.vcf"), fileName: "contacts.vcf", caption: "Sukses Tinggal Save Ya Kakak", mimetype: "text/vcard", }, { quoted: ftoko })
contacts.splice(0, contacts.length)
fs.writeFileSync("./database/contacts.json", JSON.stringify(contacts))
}
}
...
try{
let res = await fetchJson (`https://skizo.tech/api/igdl?url=${budy}&apikey=${api.Rafael}`)
for (let i of res.media) {
await sleep (100)
Rafael.sendMessage(m.chat, { react: { text: "⏳", key: m.key, }})
Rafael.sendFile(from, i,'ig.mp4', `"*乂 Instagram Downloader*"\n\n${res.caption}`, m)
	}
} catch (err){ 
let res = await instagram3 (budy)
    for (let i of res) {
await sleep (2000)
Rafael.sendMedia(from, i, ftoko)
...
try{
let res = await fetch(`https://skizo.tech/api/tiktok?url=${budy}&apikey=${api.Rafael}`)
...
Rafael.sendMessage(m.chat, { react: { text: "⏳", key: m.key, }})
  ...
  await Rafael.sendMessage(m.chat, { video: { url: json.data.play }, caption: fbk }, { quoted: ftoko })
...
Rafael.copyNForward(m.chat, msgs[budy.toLowerCase()], true)
  }
 }
    } catch (err) {
        console.log("Eror Di Bagian Rafael.js "+util.format(err))
    }
}
